...
from SSL_encoder.train_encoder_with_ssl import count_parameters
...
class SpeakerBrain(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        wav2vec_feats = self.modules.wav2vec(wavs)
        encoder_feats = self.hparams.encoder(wav2vec_feats)

        if self.hparams.wav2vec2.model.config.output_hidden_states:
            encodings_w = self.modules.weighted_sum(encoder_feats)
        else:
...
if __name__ == "__main__":
    ...
    # hparams["embedding_model"].to(params["device"]
